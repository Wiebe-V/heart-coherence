 ...
 * and pushes the result to the store. The EMA carry-over (`prevScore`) is held
 * in a ref so it never triggers a re-render, and resets whenever we leave the
 * connected state.
 */
export function useCoherence(thresholds: ZoneThresholds = DEFAULT_ZONE_THRESHOLDS): void {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (result.ready) prevScoreRef.current = result.score;
